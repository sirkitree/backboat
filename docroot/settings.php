 ...
 * Backdrop CMS may use. Backdrop CMS is able to connect to multiple databases,
 * including multiple types of databases, during the same request. These
 * advanced configuration will take precedence over the simpler $database and
 * $database_prefix variables if specified.
 *
 ...
 * @endcode
 *
 * The "driver" property indicates what Backdrop CMS database driver the
 * connection should use. This is usually the same as the name of the
 * database type, such as mysql or sqlite. The other properties will vary
 * depending on the driver.  For SQLite, you must specify a database file name
 * in a directory that is writable by the webserver.  For most other drivers,
 * you must specify a username, password, host, and database name.
 *
 * Transaction support is enabled by default for all drivers that support it,
 * including MySQL. To explicitly disable it, set the 'transactions' key to
 * FALSE. Note that some configurations of MySQL, such as the MyISAM engine,
 * don't support it and will proceed silently even if enabled. If you experience
 * transaction related crashes with such configuration, set the 'transactions'
 * key to FALSE.
 *
 * For each database, you may optionally specify multiple "target" databases.
 * A target database allows Backdrop CMS to try to send certain queries to a
 * different database if it can but fall back to the default connection if not.
 * That is useful for master/slave replication, as Backdrop CMS may try to
 * connect to a slave server when appropriate and if one is not available will
 * simply fall back to the single master server.
 *
 ...
 * of potential slave databases. Backdrop CMS will select one at random for a given
 ...
 * useful if your Backdrop CMS installation exists in a schema that is not the
 * default or you want to access several databases from the same code base at
 * the same time.
 * Example:
 * @code
 *   'prefix' => array(
